...
std::string Expr::toString() {
	...
	if (ConstantExpr *CE = dyn_cast<ConstantExpr>(this)) {
		 ...
		 * When the constant is -inf, set it to -DBL_MAX
		 */
		double val = CE->getAPValue().bitsToDouble();
		if (val > DBL_MAX)
		...
		if (this->getKind() == Expr::Concat) {
			const ReadExpr *base = dyn_cast<ReadExpr>(this->getKid(0));
			return base->updates.root->name;
		}
	}
	// Only handle
	...
	switch (this-> getKind()){
		...
		case Expr::Slt:
			sStream << "( ";
			sStream << this->getKid(0)->toString();
			sStream << " < ";
			sStream << this->getKid(1)->toString();
			sStream << ")";
			return sStream.str();
		case Expr::Ult:
			sStream << "( ";
		...
		case Expr::Eq:
			sStream << "( ";
			if (
					this->getKid(0)->getWidth()== Expr::Bool ||
					this->getKid(1)->getWidth() == Expr::Bool
			) {
				if (this->getKid(0)->isFalse()){
					sStream << " ! ";
					sStream << this->getKid(1)->toString();
				}else if (this->getKid(1)->isFalse()) {
					sStream << " ! ";
					sStream << this->getKid(0)->toString();
				}
			}else{
		...
		case Expr::Ne:
			sStream << "( ";
			if (this->getKid(0)->isTrue()){
				sStream << " ! ";
				sStream << this->getKid(1)->toString();
			}else if (this->getKid(1)->isTrue()) {
				sStream << " ! ";
				sStream << this->getKid(0)->toString();
			}else{
				sStream << this->getKid(0)->toString();
		...
		/// FIXME: Handle Select expression
		case Expr::Select:
			return "Invalid";
...
void Expr::toPolyFraction(Poly& nom, Poly& denom,Poly::CompareType& type ){
	if (ConstantExpr *CE = dyn_cast<ConstantExpr>(this)){
	...
	case Expr::Slt:
		this->getKid(0)->toPolyFraction(nom1,denom1,t1);
		this->getKid(1)->toPolyFraction(nom2,denom2,t2);
		if (isBoundary(this->getKid(0)))
			Poly::normalize(nom2, denom2);
		if (isBoundary(this->getKid(1)))
	...
	case Expr::Ult:
		...
		type = Poly::Lt;
	  	break;
	case Expr::Eq:
		if (this->getKid(0)->isBool() || this->getKid(1)->isBool()){
			if (ConstantExpr *CE = dyn_cast<ConstantExpr>(this->getKid(0))){
				if (CE->isTrue()){
					this->getKid(1)->toPolyFraction(nom,denom,type);
					return;
				}else{
					this->getKid(1)->toPolyFraction(nom,denom,type);
			...
			if(ConstantExpr *CE = dyn_cast<ConstantExpr>(this->getKid(1))){
				if (CE->isTrue()){
					this->getKid(0)->toPolyFraction(nom,denom,type);
					return;
				}else{
					this->getKid(0)->toPolyFraction(nom,denom,type);
					type = Poly::negateType(type);
...
#define BINARY_EXPR_CASE(T)                                 \
	  ...
	  //added by Thanh: Division Expression
	  BINARY_EXPR_CASE(Div);
...
void ConstantExpr::toString(std::string &Res) const {
	llvm::SmallVector<char,500> Buffer;
	fvalue.toString(Buffer,500,500);
...
ref<ConstantExpr> ConstantExpr::Neg() {
	if (this->isFP()) {
		APFloat result = APFloat(0.0);
		result.subtract(
				fvalue, APFloat::rmNearestTiesToEven
		);
...
ref<ConstantExpr> ConstantExpr::Add(const ref<ConstantExpr> &RHS) {
	if (RHS->isFP() || this->isFP()) {
		double x,y;
		x = value.bitsToDouble();
		y = RHS->value.bitsToDouble();
		APFloat v = APFloat(0.0);
		if ( x + y > DBL_MAX)
			v = APFloat(DBL_MAX);
		else if (x + y < -DBL_MAX)
			v = APFloat(-DBL_MAX);
...
ref<ConstantExpr> ConstantExpr::Div(const ref<ConstantExpr> &RHS) {
	fvalue.divide(
				RHS->fvalue, APFloat::rmNearestTiesToEven
	);
	return ConstantExpr::alloc(fvalue);
}
...
ref<ConstantExpr> ConstantExpr::Eq(const ref<ConstantExpr> &RHS) {
	if (this->isFP() || RHS->isFP()) {
		double x,y;
		x = value.bitsToDouble();
		y = RHS->value.bitsToDouble();
		return ConstantExpr::alloc(x == y, Expr::Bool);
	}
...
//added by Thanh
static ref<Expr> DivExpr_create(const ref<Expr> &l, const ref<Expr> &r) {
  if (l->getWidth() == Expr::Bool) { // r must be 1
    return l;
  } else{
    return DivExpr::alloc(l, r);
  }
}
