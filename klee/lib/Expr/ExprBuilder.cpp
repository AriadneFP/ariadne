...
namespace {
  class DefaultExprBuilder : public ExprBuilder {
	...
	//added by Thanh
	virtual ref<Expr> Div(const ref<Expr> &LHS, const ref<Expr> &RHS) {
      return DivExpr::alloc(LHS, RHS);
    }
  ...
  public:
	...
	ref<Expr> Div(const ref<Expr> &LHS, const ref<Expr> &RHS) {
      return Base->Div(LHS, RHS);
    }
  ...
  public:
	...
	virtual ref<Expr> Div(const ref<Expr> &LHS, const ref<Expr> &RHS) {
      if (ConstantExpr *LCE = dyn_cast<ConstantExpr>(LHS)) {
        if (ConstantExpr *RCE = dyn_cast<ConstantExpr>(RHS))
          return LCE->Div(RCE);
        return Builder.Div(LCE, cast<NonConstantExpr>(RHS));
      } else if (ConstantExpr *RCE = dyn_cast<ConstantExpr>(RHS)) {
        return Builder.Div(cast<NonConstantExpr>(LHS), RCE);
      }

      return Builder.Div(cast<NonConstantExpr>(LHS),
                          cast<NonConstantExpr>(RHS));
    }
