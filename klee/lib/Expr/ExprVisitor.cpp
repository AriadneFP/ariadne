...
ref<Expr> ExprVisitor::visitAriadne(const ref<Expr> &e) {
   if (!UseVisitorHash || isa<ConstantExpr>(e)) {
    return visitActual(e);
  } else {
    visited_ty::iterator it = visited.find(e);
...
ref<Expr> ExprVisitor::visitActual(const ref<Expr> &e) {
  if (isa<ConstantExpr>(e)) {    
    ...
    //added by Thanh: Floating point Division
    case Expr::Div: res = visitDiv(static_cast<DivExpr&>(ep)); break;
...
//added by Thanh
ExprVisitor::Action ExprVisitor::visitDiv(const DivExpr&) {
  return Action::doChildren(); 
}
