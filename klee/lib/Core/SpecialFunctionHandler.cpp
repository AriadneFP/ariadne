...
// FIXME: Handle GSL_ERROR to avoid abnormal terminating
...
#define addDNR(name, handler) { name, \
  ...
  // handle gsl_error
  add("gsl_error", handleGslError, true),
  // handle capture Expr
  add("captureExpr", handleCaptureExpr, true),
  //raise exception
  add("raiseException", handleRaiseException, true),
  //set location
  add("setExceptionLocation", handleSetLocation, true),
...
// Handling gsl_error
void SpecialFunctionHandler::handleGslError(ExecutionState &state,
                                           KInstruction *target,
                                           std::vector<ref<Expr> > &arguments) {
  /// At this time, we only consider PCs leading to exceptions and do not care
  /// about GSL_errors
  ///klee_warning("%s", "Input reaches the gsl_error function, i.e. this is an error that gsl handles.");
  executor.terminateStateOnExit(state);
}

// Get the current symbolic expression, update the symbolic variable table
void SpecialFunctionHandler::handleCaptureExpr(ExecutionState &state,
											   KInstruction *target,
											   std::vector<ref<Expr> > &arguments) {
  executor.handleCaptureExpr(state,target);
}

// Get current exception type that Path Constraints can generate inputs
void SpecialFunctionHandler::handleRaiseException(ExecutionState &state,
											   KInstruction *target,
											   std::vector<ref<Expr> > &arguments) {
  executor.handleRaiseException(state,target);
}

// Get current exception type that Path Constraints can generate inputs
void SpecialFunctionHandler::handleSetLocation(ExecutionState &state,
											   KInstruction *target,
											   std::vector<ref<Expr> > &arguments) {
  executor.handleSetLocation(state,target);
}
