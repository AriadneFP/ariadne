...
private:
  class TimerInfo;
  std::vector<std::string> originalVars; // List of original variables
  std::vector<std::string> newVars; // List of new variables
  KModule *kmodule;
  ...
  // Concrete value table
  // This table will store the concrete values of symbolic input variables
  // The table has the following format
  // Input variable Name 		Concrete value
  //       arr1						  1.0

  //std::map<std::string, double> concreteValues;
  ...
  //Count number of paths explored
  int numPath;

  // Store the current expression used within an elementary function
  ref<Expr> currentExp;

  //the number of variables used including temporary variable
  unsigned numVar;
  unsigned selectVars; // number of varibales created by select instructions
  unsigned inputNum; // number of input variables
  ...
  void updateSymbolicTable(
	  ExecutionState &state, const MemoryObject *mo, unsigned int id
  );

  /// The functions like sqrt are handled correctly by adding new constraint
  /// representing the relationship between dependent variable and the regular
  /// variable. This function return the array representing a symbolic variable
  /// for this symbolic variable.
  Array* getHandledVar(
  ...
  //Handle Capture expression, update variable table

  void handleCaptureExpr (ExecutionState &state, KInstruction *target);

  //Handle raise Exception, update current considering exception
  void handleRaiseException (ExecutionState &state, KInstruction *target);

  //Handle set Exception Location, update current location of the raised exception
  void handleSetLocation (ExecutionState &state, KInstruction *target);
  ...
  //Concretize random value for free varibles when the solver can not solve the constraint to set values for input varibles
  //bool concretize(ExecutionState &state, const ref<Expr> &conditions);
  ...
  void printState(ExecutionState &state, int count);
  ...
  ref<klee::ConstantExpr> toConstantSMT(ExecutionState &state, ref<Expr> e, 
                                     const char *purpose);
  ...
  void terminateState(ExecutionState &state, bool early = false);
  // call exit handler and terminate state
  void terminateStateEarly(ExecutionState &state, const llvm::Twine &message);
  // call exit handler and terminate state
  void terminateStateOnExit(ExecutionState &state);
  // call return handler and terminate state
