...
namespace {
  ...
  NoExternals("no-externals", 
           cl::desc("Do not allow external function calls"));
  ...
  cl::opt<bool>
  UseAriadne("use-ariadne",
                  cl::desc("Use Ariadne Solver to solve the constraints."),
                  cl::init(true));

  cl::opt<bool>
  ExportISAT("export-isat",
                    cl::desc("Use Ariadne Solver to solve the constraints."),
                    cl::init(true));
}
...
Solver *constructSolverChain(Solver *solver,
  ...
  if (DebugValidateSolver && !UseAriadne)
    solver = createValidatingSolver(solver, (STPSolver*)solver);
...
Executor::Executor(const InterpreterOptions &opts,
  ...
  : Interpreter(opts),
	...
	inputNum(0) {
	if (ExportISAT)
		ariadneSolver->setConstraintFormat(AriadneSolver::ISAT);
	else
		ariadneSolver->setConstraintFormat(AriadneSolver::SMTLIB);
	Solver *solver;
	if ( UseAriadne )
		solver = ariadneSolver;
	else
		solver = new STPSolver(UseForkedSTP, STPOptimizeDivides);

    solver = constructSolverChain(solver,
    ...
    this->solver = new TimingSolver(solver,
    		new STPSolver(UseForkedSTP, STPOptimizeDivides), ariadneSolver);
  ...
  if (it != seedMap.end()) {
    ...
    for (std::vector<SeedInfo>::iterator siit = it->second.begin(), 
      ...
      bool success = solver->mustBeFalse(
		  state, siit->assignment.evaluate(condition), res
      );
      assert(success && "FIXME: Unhandled solver failure");
      (void) success;
      if (res) {
...
ref<klee::ConstantExpr> Executor::evalConstant(Constant *c) {
  if (llvm::ConstantExpr *ce = dyn_cast<llvm::ConstantExpr>(c)) {
    ...
    if (const ConstantInt *ci = dyn_cast<ConstantInt>(c)) {
    	ref<klee::Expr> e;
    	e = ConstantExpr::alloc(ci->getValue());
    	ConstantExpr *ce = dyn_cast<ConstantExpr>(e);
    	return ce;
    } 

    //FIXME: Error with converting float type
	//Add floating point value to Expr
	else if (const ConstantFP *cf = dyn_cast<ConstantFP>(c)) {      
		ref<klee::Expr> e = ConstantExpr::alloc(cf->getValueAPF());
		ConstantExpr *ce = dyn_cast<ConstantExpr>(e);
		return ce;
    }	
...
ref<klee::ConstantExpr>
Executor::toConstantSMT(ExecutionState &state, 
                     ref<Expr> e,
                     const char *reason) {
  e = state.constraints.simplifyExpr(e);
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(e))
...
void Executor::executeGetValue(ExecutionState &state,
  ...
  if (it==seedMap.end() || isa<ConstantExpr>(e)) {
    ...
    for (std::vector<SeedInfo>::iterator siit = it->second.begin(), 
      ...
      ref<ConstantExpr> value;
      bool success = solver->getValue(
		  state, siit->assignment.evaluate(e), value
      );
      assert(success && "FIXME: Unhandled solver failure");
      (void) success;
      values.insert(value);
...
// Handle capture expression, update current expression within special function
// Update the variable table
void Executor::handleCaptureExpr(ExecutionState &state, KInstruction *ki ) {
	currentExp = eval(ki, 1, state).value;
}

// Handle capture expression, update current expression within special function
// Update the variable table
void Executor::handleRaiseException(ExecutionState &state, KInstruction *ki ) {
	ref<Expr> exceptionExpr = eval(ki, 1, state).value;
	if (ConstantExpr *CE = dyn_cast<ConstantExpr>(exceptionExpr)) {
		switch (CE->getZExtValue()) {
		case 0:
			exceptionHandler->setException(ExceptionHandler::Overflow);
			break;
		case 1:
			exceptionHandler->setException(ExceptionHandler::Underflow);
			break;
		case 2:
			exceptionHandler->setException(ExceptionHandler::Invalid);
			break;
		case 3:
			exceptionHandler->setException(ExceptionHandler::DivideByZero);
			break;
		default:
...
// Handle capture expression, update current expression within special function
// Update the variable table
void Executor::handleSetLocation(ExecutionState &state, KInstruction *ki ) {
	ref<Expr> locationExpr = eval(ki, 1, state).value;
	if (ConstantExpr *CE = dyn_cast<ConstantExpr>(locationExpr)) {
		exceptionHandler->setLocation(CE->getZExtValue());
	}
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Select: {
    ...
    if (SelectExpr *selectExpr = dyn_cast< SelectExpr >(result)) {
    	selectVars++;
    	selectExpr->setSymbolicIndex(selectVars);
    }
    bindLocal(ki, state, result);
    break;
  }
  ...
  case Instruction::FDiv: {
    ref<Expr> left = eval(ki, 0, state).value;
    ref<Expr> right = eval(ki, 1, state).value;
    ref<Expr> result = DivExpr::create(left, right);
    bindLocal(ki, state, result);
    break;
  }
  ...
  case Instruction::FPTrunc: {
    ...
    if (arg.isNull()) {
       	bindLocal(ki, state, ConstantExpr::alloc(0, resultType));
       	break;
    }
...
void Executor::terminateState(ExecutionState &state, bool early) {
  if (replayOut && replayPosition!=replayOut->numObjects) {
    klee_warning_once(replayOut, 
                      "replay did not consume all objects in test input.");
  }
  if (!early)
	  numPath++; // This is the total number of paths explored
...
/// When terminating the state early, do not generate test cases.
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(size)) {
    ...
    while (example->Ugt(ConstantExpr::alloc(128, W))->isTrue()) {
      ...
      bool success = solver->mayBeTrue(
		  state, EqExpr::create(tmp, size), res
      );
      assert(success && "FIXME: Unhandled solver failure");      
      (void) success;
      if (!res)
...
void Executor::executeMemoryOperation(ExecutionState &state,
  ...
  if (success) {
    ...
    bool inBounds;
    solver->setTimeout(stpTimeout);
    bool success = solver->mustBeTrue(state, mo->getBoundsCheckOffset(offset, bytes),
	                              inBounds);
    solver->setTimeout(0);
    if (!success) {
      state.pc = state.prevPC;
...
Array* Executor::getHandledVar(
  	  ExecutionState &state, const MemoryObject *mo
) {
	std::string varName = mo->getName();
	std::string prefix = std::string("__sqrt");
	size_t found = varName.find(prefix);
	/// Case sqrt function
	if (found != std::string::npos) {
		static unsigned sqrtIndex = 0;
		Array *array = new Array("sqrtArr" + llvm::utostr(++sqrtIndex), mo->size);
		ref<Expr> dependentVar = Expr::createTempRead( array, Expr::Int64 );
		ref<Expr> varSquare = MulExpr::create( dependentVar, dependentVar );
		ref<Expr> condition = EqExpr::create( varSquare, currentExp );
		state.addConstraint(condition);
		return array;
	}
	return NULL;
...
void Executor::updateSymbolicTable(
	ExecutionState &state, const MemoryObject *mo, unsigned int id
) {
	/*
	 * Handle elementary functions: partition symbolic variables, original
	 * variables and new variables. The dependent symbolic variables represent
	 * the value for the call to elementary functions. With the function that
	 * we can represent the semantics precisely, we keep both of symbolic variables.
	 * For other functions, we concretize all of variables in the constraints
	 * containing dependent variables.
	 * Original variable have name beginning __ariadne_tmp
	 * New variable have name beginning __new_ariadne_tmp
	 */

	std::string newVar = std::string("__ariadne_tmp");
	std::string name = mo->getName();
	std::string var = std::string("arr" + llvm::utostr(id));
	size_t found;
	found = name.find(newVar);
	if (found != std::string::npos) {
		originalVars.push_back(var);
		inputNum++;
		ariadneSolver->setInputNum(inputNum);
	} else {
		newVars.push_back("arr" + llvm::utostr(id));
		std::string prefix = std::string("__exp");
		found = name.find(prefix);
		// Case exp function
		// Function index is 2
		if (found != std::string::npos) {
			std::pair<int, ref<Expr> > expression(2, currentExp);
			std::pair<std::string, std::pair<int, ref<Expr> > > tuple(var,
					expression);
			varTable.insert(tuple);
		}
		prefix = std::string("__log");
		found = name.find(prefix);
		// Case log function
		// Function index is 3
		if (found != std::string::npos) {
			std::pair<int, ref<Expr> > expression(3, currentExp);
			std::pair<std::string, std::pair<int, ref<Expr> > > tuple(var,
					expression);
			varTable.insert(tuple);
		}

		prefix = std::string("__sin");
		found = name.find(prefix);
		// Case sin function
		// Function index is 4
		if (found != std::string::npos) {
			std::pair<int, ref<Expr> > expression(4, currentExp);
			std::pair<std::string, std::pair<int, ref<Expr> > > tuple(var,
					expression);
			varTable.insert(tuple);
		}

		prefix = std::string("__cos");
		found = name.find(prefix);
		// Case cos function
		// Function index is 5
		if (found != std::string::npos) {
			std::pair<int, ref<Expr> > expression(5, currentExp);
			std::pair<std::string, std::pair<int, ref<Expr> > > tuple(var,
					expression);
			varTable.insert(tuple);
		}
		ariadneSolver->setVarTable(varTable);
	}
}
...
void Executor::executeMakeSymbolic(ExecutionState &state,
  ...
  if (!replayOut) {
    ...
    Array* array = getHandledVar(state, mo);
    if (!array) {
    	// Create a new object state for the memory object (instead of a copy).
    	numVar++;
    	ariadneSolver->setNumVar(numVar);
    	array = new Array("arr" + llvm::utostr(++id), mo->size);
    	updateSymbolicTable(state, mo, id);
    }
...
bool Executor::getSymbolicSolution(const ExecutionState &state,
  ...
  ariadneSolver->printTestCase(); // Set print flag to true
  bool success = solver->getInitialValues(tmp, objects, values);
  ariadneSolver->notPrintTestCase(); // Set print flag to false

  solver->setTimeout(0);
  if (!success) {
	  std::cout << "Unable to compute initial values (invalid constraints?)!"
			    << std::endl;
  }
  else {
	  for (unsigned i = 0; i != state.symbolics.size(); ++i)
