...
#include "klee/CexCachingSolver.h"
...
bool CexCachingSolver::getAssignment(const Query& query, Assignment *&result) {
  ...
  if (lookupAssignment(query, key, result)) {
	  /// Print out the test cases if we can find an assignment for the query
	  if (
		exceptionHandler->getException() != ExceptionHandler::NotException
		&& result && exceptionHandler->check(*result)
	  ) {
		  result->print();
		  /// After exporting the test cases, reset the exception type
