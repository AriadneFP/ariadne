...
#include "klee/util/ExprPPrinter.h" /// debugging
...
IndependentElementSet getIndependentConstraints(const Query& query,
  ...
  do {
    ...
    for (std::vector< std::pair<ref<Expr>, IndependentElementSet> >::iterator
      ...
      // FIXME: change the logic to add expression to the result. Whenever we
      // cannot find an intersected expression, we add it to the result.
    ...
    // std::cout << newWorklist.size() << std::endl; // debugging
