...
//Using for converting to Rational form
#include "klee/Poly.h"
...
public:
  ...
  enum Kind {
    ...
    UDiv,
    SDiv,
	//Ariadne specific to handle floating point division
	Div,
    URem,
    SRem,
...
public:
  ...
  void toPolyFraction(Poly &p,Poly &q,Poly::CompareType& type);

  //Convert expression to arithmetic format for Function Parser
  std::string toString();
  bool isBool() const{
	  if (this->getWidth() == Expr::Bool)
		  return true;
	  else
		  return false;
  }
...
private:
  ...
  ConstantExpr(const llvm::APInt &v) : value(v),fvalue(llvm::APFloat(v.bitsToDouble())),isFloat(false) {}
  ConstantExpr(const llvm::APFloat &v) : value(v.bitcastToAPInt()), fvalue(v), isFloat(true) {}
...
public:
  ...
  llvm::APFloat getFloat() const { return fvalue; }

  bool isFP() const { return isFloat;}

  void setFP() {
	  isFloat = true;
	  fvalue = llvm::APFloat(value.bitsToDouble());
  }
  ...
  static ref<ConstantExpr> alloc(const llvm::APFloat &v) {
    ref<ConstantExpr> r(new ConstantExpr(v));
    r->computeHash();
    return r;
  } 
  ...
  ref<ConstantExpr> Div(const ref<ConstantExpr> &RHS);	
...
public:
  ...
  int getSymbolicIndex() const {
	  return symbolicIndex;
  }

  void setSymbolicIndex(int index) {
	  symbolicIndex = index;
  }
