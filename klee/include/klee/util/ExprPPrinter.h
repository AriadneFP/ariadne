...
namespace klee {
  ...
  public:
    ...
    virtual void printSMT_LIB(const ref<Expr> &e, unsigned indent=0) = 0;
    ...
    //print constraints to a SMT-LIB standard file	
    static void printSMT_LIB (std::ostream &os,
						const ConstraintManager &constraints);

    /// Print constraints as ISAT format
    static void print_iSAT (std::ostream &os,
						const ConstraintManager &constraints);

    /// Print constraints as Z3 internal format
    static void print_Z3 (std::ostream &os,
    						const ConstraintManager &constraints);

    //Print the constraint in Polynomial forms
    static void print_Poly (std::ostream &os,
						const ConstraintManager &constraints);

    //Print single constraint
    static void printSingleExpr_Poly(std::ostream &os,
						const ref<Expr> &e);

    static void printQuery_SMT(std::ostream &os,
                           const ConstraintManager &constraints,
                           const ref<Expr> &q,
