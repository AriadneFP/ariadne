...
extern cl::opt<unsigned int> AriadneTime;
...
void KleeHandler::processTestCase(const ExecutionState &state,
  ...
  /// Only generate test cases for PCs leading to exceptions
  if (exceptionHandler->getException() == ExceptionHandler::NotException)
	  return;
  ...
  if (!NoOutput) {
    ...
    //Should move the number of test cases here
    unsigned id = m_testIndex+1;
    ...
    //FIXME: at this time, do not use this options
...
//#endif
  ...
  // Ariadne injected functions
  "raiseException",
  "setExceptionLocation",
  "captureExpr",
  // GSL external functions
  "gsl_error"
};
...
void printStats() {
	...
	uint64_t queriesValid = *theStatisticManager->getStatisticByName(
			"QueriesValid");
	uint64_t queriesInvalid = *theStatisticManager->getStatisticByName(
			"QueriesInvalid");
	uint64_t queryCounterexamples = *theStatisticManager->getStatisticByName(
			"QueriesCEX");
	uint64_t queryConstructs = *theStatisticManager->getStatisticByName(
			"QueriesConstructs");
	uint64_t instructions = *theStatisticManager->getStatisticByName(
			"Instructions");
	...
	statsStr << "\n";
	statsStr << "KLEE: done: total instructions = " << instructions << "\n";
	statsStr << "KLEE: done: completed paths = "
			<< handler->getNumPathsExplored() << "\n";
	statsStr << "KLEE: done: generated tests = " << handler->getNumTestCases()
			<< "\n";
	statsStr << "KLEE: done: Z3 queries = "
			<< ariadneSolver->getNumofZ3_queries() << "\n";
	statsStr << "KLEE: done: Z3 sat queries = "
			<< ariadneSolver->getNumofZ3_sat_queries() << "\n";
	statsStr << "KLEE: done: Z3 unsat queries = "
			<< ariadneSolver->getNumofZ3_unsat_queries() << "\n";
	statsStr << "KLEE: done: Poly queries = "
			<< ariadneSolver->getNumofPoly_queries() << "\n";
	statsStr << "KLEE: done: Poly sat queries = "
			<< ariadneSolver->getNumofPoly_sat_queries() << "\n";
	statsStr << "KLEE: done: Poly sat single queries = "
			<< ariadneSolver->getNumofPoly_single_sat_queries() << "\n";
	statsStr << "KLEE: done: Poly sat multiple queries = "
			<< ariadneSolver->getNumofPoly_multiple_sat_queries() << "\n";
	statsStr << "KLEE: done: Poly unsat queries = "
			<< ariadneSolver->getNumofPoly_unsat_queries() << "\n";
	statsStr << "KLEE: done: Poly unsat single variable queries = "
			<< ariadneSolver->getNumofPoly_single_unsat_queries() << "\n";
	statsStr << "KLEE: done: Poly unsat multiple variable queries = "
			<< ariadneSolver->getNumofPoly_multiple_unsat_queries() << "\n";
	double rate;

	if (ariadneSolver->getSuccessfulConcretization()
			+ ariadneSolver->getFailedConcretization() == 0)
		rate = 0;

	else {
		rate = double(ariadneSolver->getSuccessfulConcretization())
				/ (double) (ariadneSolver->getSuccessfulConcretization()
						+ ariadneSolver->getFailedConcretization());
		rate = rate * 100;
	}

	statsStr << "KLEE: done: Number of successful concretization: "
			<< ariadneSolver->getSuccessfulConcretization() << endl;
	statsStr << "KLEE: done: Number of failed concretization: "
			<< ariadneSolver->getFailedConcretization() << endl;
	statsStr << "KLEE: done: Successful rate of concretization: " << rate
			<< "%" << endl;
	statsStr << "KLEE: done: Number of cache hits = " << stats::queryCacheHits
			<< "\n";
	statsStr << "KLEE: done: Number of cache misses = "
			<< stats::queryCacheMisses << "\n";
	statsStr << "KLEE: done: Number of cex cache hits = "
			<< stats::queryCexCacheHits << "\n";
	statsStr << "KLEE: done: Number of cex cache misses = "
			<< stats::queryCexCacheMisses << "\n";

	std::cerr << statsStr.str();
	handler->getInfoStream() << statsStr.str();
}
...
static void interrupt_handle() {
  if (!interrupted && theInterpreter) {
    ...
    /// This function must appear on both arms of the if because of the exit
    /// on the else
  } else {
...
int main(int argc, char **argv, char **envp) {
  ...
  if (ariadneSolver == NULL) {
	  ariadneSolver = new AriadneSolver();
	  /// Convert timeout from seconds to mili-seconds
	  ariadneSolver->setTimeout(AriadneTime * 1000);
  }
    ...
    if (pid<0) {
      ...
      while (1) {
        ...
        if (res < 0) {
          ...
          double kTime = util::getWallTime();

          if (kTime > nextStep) {
            ++level;
            
            ifstream logFile("gsl-test.log");
            ...
            /// When we timeout the whole process, we might be in the middle of
            /// querying constraints, we let the current query to finish before
            /// terminating the whole process.

            bool evaluatingZ3 = true;
            if (
            	lastLine.find("Running") != string::npos ||
            	lastLine.find("After") != string::npos ||
            	lastLine.find("KLEE: WATCHDOG") != string::npos
            ) {
            	evaluatingZ3 = false;
            }
            if ( evaluatingZ3 ) {
            	string prefix = "Before z3 check 1: ";
            	int prefixLength = prefix.size();
            	lastLine = lastLine.replace( 0, prefixLength, "" );
            	stringstream startTimeSS(lastLine);
            	int startTime;
            	startTimeSS >> startTime;
            	time_t currentTime;
            	time(&currentTime);
            	stringstream currentTimeSS;
            	currentTimeSS << currentTime << endl;
            	int cTime;
            	currentTimeSS >> cTime;
            	unsigned int delta  = cTime - startTime;
            	if (delta < AriadneTime)
            		sleep(AriadneTime + 1 - delta); /// Let the current query finish*/
            }
            fstream filestr;
            filestr.open("gsl-test.log", fstream::out | fstream::app);

            if (level==1) {
              fprintf(stderr, "KLEE: WATCHDOG: time expired, attempting halt via INT\n");
              filestr << "KLEE: WATCHDOG: time expired, attempting halt via INT" << endl;
              kill(pid, SIGINT);
            } else if (level==2) {
              fprintf(stderr, "KLEE: WATCHDOG: time expired, attempting halt via gdb\n");
              filestr << "KLEE: WATCHDOG: time expired, attempting halt via gdb" << endl;
              halt_via_gdb(pid);
            } else {
              fprintf(stderr, "KLEE: WATCHDOG: kill(9)ing child (I tried to be nice)\n");
              filestr << "KLEE: WATCHDOG: kill(9)ing child (I tried to be nice)" << endl;
              kill(pid, SIGKILL);
  ...
  if (useGSL) {
	  llvm::sys::Path GSLPath(Opts.LibraryDir);
	  GSLPath.appendComponent("../../../Test/gsl-1.14/.libs/libgsl.a");
	  mainModule = klee::linkWithLibrary(mainModule, GSLPath.c_str());
	  assert(mainModule && "unable to link with GSL library");
	  llvm::sys::Path CBLASPath(Opts.LibraryDir);
	  CBLASPath.appendComponent("../../../Test/gsl-1.14/cblas/.libs/libgslcblas.a");
	  mainModule = klee::linkWithLibrary(mainModule, CBLASPath.c_str());
	  assert(mainModule && "unable to link with GSLCBLAS library");
  }
